...
class NPuzzle(EnvGrndAtoms[NPState, NPAction, NPGoal], EnvStartGoalRW[NPState, NPAction, NPGoal],
              EnvEnumerableActs[NPState, NPAction, NPGoal], EnvVizable, EnvSupportsPDDL):
  ...
  (:action move-right
    ...
    def state_goal_to_pddl_inst(self, state: State, goal: Goal) -> List[str]:
        if (not isinstance(state, NPState)) or (not isinstance(goal, NPGoal)):
