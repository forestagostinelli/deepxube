from typing import List, Dict, Tuple, Any, TypeVar, Generic, cast
...
SEARCH = TypeVar('SEARCH', bound=Search)


class Updater(ABC, Generic[SEARCH]):
    def __init__(self, env: Environment):
        self.env: Environment = env

    @classmethod
    def update_runner(cls, gen_step_max: int, search_step_max: int, env: Environment, heur_fn_par: NNetPar,
                      nnet_type: NNetType, to_q: Queue, from_q: Queue, step_probs: NDArray,
        ...
        while True:
            ...
            for _ in range(search_step_max):
                ...
                # to nnet
                start_time = time.time()
                inputs_nnet, ctgs_bellman = cls.step_get_data(search, times)
                ...
                if isinstance(search, SearchV):
                    ...
                    states, goals, actions, ctgs_bellman = cast(
    ...
    @staticmethod
    @abstractmethod
    def step_get_data(search: SEARCH, times: Times) -> Tuple[List[NDArray], List[float]]:
        pass
