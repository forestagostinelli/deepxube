...
from deepxube.updaters.updater_rl_q import UpdateHeurRLQEnum
from deepxube.updaters.updater_rl_v import UpdateHeurRLV
...
def get_updater(domain: Domain, heur_nnet_par: HeurNNetPar, pathfind_name: str, pathfind_kwargs: Dict[str, Any], up_args: UpArgs,
    ...
    if issubclass(pathfind_t, PathFindHeur):
        ...
        updater: UpdateHeurRL
        if isinstance(heur_nnet_par, HeurNNetParV):
            updater = UpdateHeurRLV(domain, pathfind_name, pathfind_kwargs, up_args, up_heur_args)
        elif isinstance(heur_nnet_par, HeurNNetParQ):
            updater = UpdateHeurRLQEnum(domain, pathfind_name, pathfind_kwargs, up_args, up_heur_args)
        else:
